...
APMC_adj_origin_q <- function(N, alf, paccmin, sim_coords,  observed_data, param,  
                              m, 
                              k, 
  ...
  pb <- progress_bar$new(
    format = "  progress [:bar] :percent | time used: :elapsed | estimated rest time: :eta",
    total = 4,
    clear = FALSE,
    width = 60
  )
  ...
  distances <- sapply(sim_scores_list, function(sim_score) {
    ...
    # Sum columns for both observed and simulated scores
    obs_sums <- colSums(observed_batched_scores)
    sim_sums <- colSums(sim_score)
    
    # Convert to matrix format
    obs_vector <- matrix(obs_sums, ncol=1)
    sim_vector <- matrix(sim_sums, ncol=1)
    
    # Use only the subset corresponding to optimization parameters
    hatCholJ_subset <- hatCholJ[opt_indices, opt_indices]
    
    # Whiten the summed vectors
    whitened_obs <- hatCholJ_subset %*% obs_vector
    whitened_sim <- hatCholJ_subset %*% sim_vector
    
    # Calculate difference and return scalar distance
    diff_vec <- whitened_obs - whitened_sim
    return(sqrt(sum(diff_vec^2)))
  })
  

  valid_indices <- which(!is.na(distances))  # 获取非 NA 的索引
  ...
  while (current_iter <= max_iter && ACC[current_iter] > paccmin) {
    ...
    message(sprintf("\n========== main iteration %d  ==========", current_iter - 1))
    ...
    message(sprintf("Tolerance level: %.4f", eps))
    message(sprintf("Number of valid particles: %d", sum(!invalid_particles)))
    message("Mean of parameters to be optimized:")
    print(colMeans(theta[, opt_indices, drop = FALSE]))
    message("Standard deviation of parameters to be optimized:")
    ...
    if (!is.null(sd_old)) {
      ...
      if (rel_change < sd_tol) {
        message("Standard deviation converged, early stopping.")
    ...
    # Print diagnostics every 5 iterations
    if(current_iter %% 5 == 0) {
      cat("\n==== Iteration", current_iter, "Diagnostics ====\n")
      cat("Change rate of parameter SD:", 
          if(current_iter > 1) {
      ...
      cat("Acceptance rate:", diagnostics_data[[current_iter]]$acceptance, "\n")
      cat("Weight range:", diagnostics_data[[current_iter]]$weightMin, 
          "to", diagnostics_data[[current_iter]]$weightMax, "\n")
    }
...
#is.param <- list(mean=mod.spe1$fitted.values, vcov=matrix(2*mod.spe1$var.cov, 10, 10))



rcs102<- APMC_adj_origin_q(
  N = 1000,
  alf = 0.3,
  paccmin = 0.01,
